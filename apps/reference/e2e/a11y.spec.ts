...
test.describe('Accessibility', () => {
	...
	test('auth sign-in page has no critical a11y violations', async ({
		page,
	}) => {
	...
	test('theme contrast: base theme passes color contrast', async ({
		page,
	}) => {
		await page.goto('/');
		const results = await new AxeBuilder({ page })
	...
	test('theme contrast: deepwood theme passes color contrast', async ({
		page,
	}) => {
		await page.goto('/');
		await page.evaluate(() => {
